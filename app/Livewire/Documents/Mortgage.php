...
class Mortgage extends Component
{
    ...
    {
        return view('livewire.documents.mortgage', [
            'loans' => Loan::query()
    ...
    public function exportSingleMortgage($loanId)
    {
        $loan = Loan::findOrFail($loanId);

        // Dispatch browser event to trigger download
        $this->dispatch('download-pdf', [
            'url' => route('loan.mortgage.export', $loan->id)
        ]);
     ...
     public function exportFilteredAgreements()
    {
        ...
        $this->dispatch('download-bulk-pdf', [
            'url' => route('loan.mortgage.bulk-export'),
