...
class PromissoryOrigin extends Component
{
    ...
    {
        return view('livewire.documents.promissory-origin',[
            'loans' => Loan::query()
    ...
    public function exportSinglePromissoryOrigin($loanId)
    {
        $loan = Loan::findOrFail($loanId);

        // Dispatch browser event to trigger download
        $this->dispatch('download-pdf', [
            'url' => route('loan.promissoryOrigin.export', $loan->id)
        ]);
    ...
    public function exportFilteredPromissoryOrigin()
    {
        ...
        $this->dispatch('download-bulk-pdf', [
            'url' => route('loan.promissoryOrigin.bulk-export'),
